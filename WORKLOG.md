...
- Prepared the notebooks for submission: removed the internal branch pointers from the headers of 01, 02 and 03, the "Before you commit" housekeeping cell in 02 (git, nbconvert, black, ruff), the line telling teammates to write their own "Your turn" answers, the "paste into claims_table" comment, and the in-prose references to `CLAUDE.md`. The two "Your turn" cells are now plain prose, keeping the reasoning and dropping the worksheet framing. Kept the `RULES.md` data-rule notes, which evidence compliance rather than being internal notes, and the per-notebook owner line.
- Added `cd11b` to notebook 02: cumulative positives divided by population. It prints 39 of 50 districts above one confirmed case per resident (max 7.13) and 7 above one per resident per year, matching `scripts/verify_claims.py` section E. The figure was quoted in the A1 prose but computed only in the script, which broke the notebook's own stated rule that every number is printed by the cell above it.
**Decided:**
