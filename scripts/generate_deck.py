...
def create_deck():
    ...
    # Ashesi University Official Brand Palette
    # Primary: Pale Carmine (#A83F43)
    c_ashesi_red = RGBColor(168, 63, 67)     # #A83F43 - Ashesi Pale Carmine
    c_ashesi_dark = RGBColor(94, 18, 20)     # #5E1214 - Ashesi Deep Burgundy
    c_ashesi_gold = RGBColor(197, 155, 39)   # #C59B27 - Academic Warm Gold
    c_charcoal = RGBColor(30, 32, 34)        # #1E2022 - High-contrast text
    c_slate = RGBColor(90, 98, 106)          # #5A626A - Subtitle / muted text
    c_card_bg = RGBColor(248, 249, 250)      # #F8F9FA - Card fill
    c_card_border = RGBColor(226, 228, 232)  # #E2E4E8 - Subtle border
    c_white = RGBColor(255, 255, 255)
    c_gain_blue = RGBColor(43, 108, 176)     # #2B6CB0 - Gainers
    c_loss_red = RGBColor(179, 57, 57)       # #B33939 - Losers

    # Brand Typography
    font_title = "Garamond"
    font_body = "Candara"

    logo_path = Path("figures/ashesi_logo.png")

    def add_header(slide, title_text, category_text="", slide_num=None):
        # 1. Top Ashesi Brand Accent Strip
        strip = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(13.333), Inches(0.08))
        strip.fill.solid()
        strip.fill.fore_color.rgb = c_ashesi_red
        strip.line.fill.background()

        # 2. Header Text Box
        header_box = slide.shapes.add_textbox(Inches(0.8), Inches(0.4), Inches(10.0), Inches(0.95))
        ...
        if category_text:
            ...
            p0.font.size = Pt(10)
            p0.font.bold = True
            p0.font.color.rgb = c_ashesi_red
            p1 = tf.add_paragraph()
        else:
            p1 = p0
        p1.text = title_text
        ...
        p1.font.size = Pt(23)
        p1.font.bold = True
        p1.font.color.rgb = c_ashesi_dark

        # 3. Ashesi Watermark Logo in Header (Top Right)
        if logo_path.exists():
            slide.shapes.add_picture(str(logo_path), Inches(11.8), Inches(0.35), height=Inches(0.75))
        ...
        pf = tff.paragraphs[0]
        pf.text = "Ashesi University · ICS553 Machine Learning Essentials · Group 3 · Prosit 1 Advisory Brief"
        pf.font.name = font_body
        pf.font.size = Pt(9)
        pf.font.color.rgb = c_slate

        if slide_num:
            pf2 = tff.add_paragraph()
            pf2.text = str(slide_num)
            pf2.font.name = font_body
            pf2.font.size = Pt(9)
            pf2.font.color.rgb = c_slate
            pf2.alignment = PP_ALIGN.RIGHT

    # -------------------------------------------------------------
    # SLIDE 1: Title Slide (Official Ashesi Executive Styling)
    # -------------------------------------------------------------
    s1 = prs.slides.add_slide(blank_layout)
    bg1 = s1.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(13.333), Inches(7.5))
    bg1.fill.solid()
    bg1.fill.fore_color.rgb = c_ashesi_dark
    bg1.line.fill.background()

    # Gold accent line on title slide
    gold_line = s1.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(1.0), Inches(1.5), Inches(0.08), Inches(4.5))
    gold_line.fill.solid()
    gold_line.fill.fore_color.rgb = c_ashesi_gold
    gold_line.line.fill.background()

    # Title Card
    tb1 = s1.shapes.add_textbox(Inches(1.4), Inches(1.5), Inches(7.8), Inches(4.5))
    tf1 = tb1.text_frame
    tf1.word_wrap = True
    ...
    p2.text = "An Uncertainty-Aware Decision Framework for Allocating 50,000 Indivisible Nets Across 50 Northern Districts"
    p2.font.name = font_body
    p2.font.size = Pt(17)
    p2.font.color.rgb = RGBColor(240, 220, 220)
    p2.space_before = Pt(14)

    p3 = tf1.add_paragraph()
    p3.text = "Advisory Presentation to the National Malaria Elimination Programme (NMEP)\nMinistry of Health, Republic of Ghana\n\nGroup 3 · Master of Science in Intelligent Computing Systems (MICS 2028)\nAshesi University"
    p3.font.name = font_body
    p3.font.size = Pt(13)
    p3.font.color.rgb = c_ashesi_gold
    p3.space_before = Pt(28)

    # Official Logo in White Badge on Right
    if logo_path.exists():
        badge = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(9.6), Inches(2.2), Inches(2.8), Inches(3.0))
        badge.fill.solid()
        badge.fill.fore_color.rgb = c_white
        badge.line.color.rgb = c_ashesi_gold
        badge.line.width = Pt(1.5)
        s1.shapes.add_picture(str(logo_path), Inches(9.8), Inches(2.5), width=Inches(2.4))

    # -------------------------------------------------------------
    # SLIDE 2: Problem Statement & Operational Constraints
    # -------------------------------------------------------------
    s2 = prs.slides.add_slide(blank_layout)
    add_header(s2, "The Decision Context: Allocating 50,000 Indivisible Nets", "Executive Summary", 2)

    # Card 1: Operational Mandate
    c1 = s2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.5), Inches(5.6), Inches(5.3))
    c1.fill.solid()
    ...
    p.text = "Operational Mandate & Constraints"
    p.font.name = font_title
    p.font.size = Pt(17)
    p.font.bold = True
    p.font.color.rgb = c_ashesi_dark

    bullets1 = [
        ("Strict Resource Ceiling:", " Exactly 50,000 long-lasting ITNs available for allocation."),
        ("Operational Scope:", " 50 administrative districts across Northern, Upper East, and Upper West regions."),
        ("Physical Indivisibility:", " Integer allocations required (no fractional nets; resolved via Hamilton Apportionment)."),
        ("Asymmetric Minimax Loss:", " Under-allocating to an epidemic zone costs lives; over-allocating incurs only minor warehousing friction. We evaluate risk at the 95% upper bound.")
    ]
    for b_title, b_desc in bullets1:
        ...
        r1.font.bold = True
        r1.font.size = Pt(12)
        r1.font.color.rgb = c_ashesi_dark
        r2 = p.add_run()
        r2.text = b_desc
    ...
    # Card 2: The Naive Traps
    c2 = s2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.9), Inches(1.5), Inches(5.6), Inches(5.3))
    c2.fill.solid()
    ...
    p.text = "Why The Naive Heuristic Fails"
    p.font.name = font_title
    p.font.size = Pt(17)
    p.font.bold = True
    p.font.color.rgb = c_loss_red
    ...
    bullets2 = [
        ...
        ("Satiation & Diminishing Utility:", " Upper East already has 79.6% net coverage. Dumping nets there gives zero marginal benefit."),
    ...
    add_header(s3, "Geospatial Surveillance & Survey Data Divide", "Theme B1 Geospatial EDA", 3)
    ...
    if p_map2.exists():
        s3.shapes.add_picture(str(p_map2), Inches(5.4), Inches(1.5), width=Inches(4.4))

    # Insights Card
    c3 = s3.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(10.0), Inches(1.5), Inches(2.55), Inches(5.3))
    c3.fill.solid()
    c3.fill.fore_color.rgb = c_card_bg
    c3.line.color.rgb = c_card_border

    tb_c3 = s3.shapes.add_textbox(Inches(10.15), Inches(1.7), Inches(2.25), Inches(4.9))
    tf_c3 = tb_c3.text_frame
    tf_c3.word_wrap = True
    p = tf_c3.paragraphs[0]
    p.text = "Cartographic Insights"
    p.font.name = font_title
    p.font.size = Pt(14)
    p.font.bold = True
    p.font.color.rgb = c_ashesi_dark
    ...
    insights = [
        ...
        "2. Survey Density: DHS 2022 sampled 618 clusters nationwide, but rural northern clusters carry wide sampling errors.",
        "3. Survey Weighting: Unweighted net ownership (70.96%) overstates true weighted ownership (66.77%) by 4.19 percentage points."
    ]
    for ins in insights:
    ...
    add_header(s4, "Over-Dispersion: Why Poisson Regression Crashed", "Theme A Statistical Modeling", 4)

    # Left: Explanation Card
    c4 = s4.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.5), Inches(5.5), Inches(5.3))
    c4.fill.solid()
    c4.fill.fore_color.rgb = c_card_bg
    c4.line.color.rgb = c_card_border
    ...
    p.text = "Mathematical Proof of Poisson Failure"
    p.font.name = font_title
    p.font.size = Pt(17)
    p.font.bold = True
    p.font.color.rgb = c_ashesi_dark
    ...
    points4 = [
        ("The Equidispersion Assumption:", " Poisson requires Variance = Mean (Ratio = 1.0)."),
        ("Extreme Real-World Dispersion:", " In northern Ghana, Variance / Mean = 77,183! Observed SD is 122,882 vs Poisson SD of 442 (278x too narrow!)."),
        ...
        ("Negative Binomial (NB2):", " Adds dispersion parameter (alpha = 0.2677), allowing Var = mu + alpha*mu^2. Beats Poisson by 2,393,669 AIC points!")
    ]
    for t, d in points4:
        ...
        r1.font.bold = True
        r1.font.size = Pt(12)
        r1.font.color.rgb = c_ashesi_red
        r2 = p.add_run()
        r2.text = d
        r2.font.name = font_body
        r2.font.size = Pt(11)
    ...
    # -------------------------------------------------------------
    # SLIDE 5: Theme A: Survey Uncertainty & Cluster Bootstrap
    # -------------------------------------------------------------
    s5 = prs.slides.add_slide(blank_layout)
    add_header(s5, "Honest Uncertainty: The Two-Stage Cluster Bootstrap", "Theme A Sampling Theory", 5)

    c5 = s5.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.5), Inches(5.5), Inches(5.3))
    c5.fill.solid()
    c5.fill.fore_color.rgb = c_card_bg
    c5.line.color.rgb = c_card_border
    ...
    p.text = "Survey Design Effects & False Precision"
    p.font.name = font_title
    p.font.size = Pt(17)
    p.font.bold = True
    p.font.color.rgb = c_ashesi_dark
    ...
    points5 = [
        ("Two-Stage Sampling Structure:", " DHS samples clusters (villages) first, then 25-30 households per cluster."),
        ("Intra-Cluster Correlation:", " Families in the same village share the same standing water and risk. They are NOT independent."),
        ("Naive Bootstrap Danger:", " Resampling households independently pretends you have 17,933 independent facts, shrinking CI to 7.17 pp (false precision!)."),
        ("Honest Cluster Bootstrap:", " Resampling clusters first gives a 15.20 pp interval — 2.12x wider (DEFF = 4.50)!"),
    ...
    # -------------------------------------------------------------
    # SLIDE 6: Theme B: Pipeline Rigour & Leakage Audit
    # -------------------------------------------------------------
    s6 = prs.slides.add_slide(blank_layout)
    add_header(s6, "Pipeline Discipline & The Leakage Audit", "Theme B Pipeline Engineering", 6)
    ...
    tb_b1 = s6.shapes.add_textbox(Inches(0.95), Inches(1.8), Inches(3.4), Inches(4.7))
    ...
    tb_b2 = s6.shapes.add_textbox(Inches(4.95), Inches(1.8), Inches(3.4), Inches(4.7))
    ...
    tb_b3 = s6.shapes.add_textbox(Inches(8.95), Inches(1.8), Inches(3.4), Inches(4.7))
    ...
    add_header(s7, "The Equitable Need-Weighted Allocation Policy", "Theme C Resource Optimization", 7)
    ...
    fbox.fill.solid()
    fbox.fill.fore_color.rgb = c_ashesi_dark
    ...
    p_fb.font.bold = True
    p_fb.font.color.rgb = c_ashesi_gold
    ...
    add_header(s8, "Map Highlight: 50,000 Net Distribution Across Northern Ghana", "Theme C Geospatial Allocation", 8)
    ...
    card8.line.color.rgb = c_card_border
    ...
    add_header(s9, "Map Highlight: Net Shifts (Equitable vs. Naive)", "Theme C Policy Impact", 9)
    ...
    # -------------------------------------------------------------
    # SLIDE 10: Deep Dive: The Referral Hospital Trap Explained
    # -------------------------------------------------------------
    s10 = prs.slides.add_slide(blank_layout)
    add_header(s10, "Deep Dive: The Referral Hospital Trap Explained", "Theme C Policy Defense", 10)
    ...
    c10_1.line.color.rgb = c_card_border
    ...
    c10_2.line.color.rgb = c_card_border
    ...
    add_header(s11, "Implementation Roadmap for NMEP Rollout", "Theme D Operational Strategy", 11)
    ...
    add_header(s12, "Viva Oral Defense: Examiner Grill-Me Cheat Sheet", "Academic Defense Bank", 12)
